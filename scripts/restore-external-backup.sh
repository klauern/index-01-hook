...
link_no_follow "$backup_path" "$snapshot_dir/backup.age" >/dev/null 2>&1 || fail
link_no_follow "$checksum_path" "$snapshot_dir/backup.sha256" >/dev/null 2>&1 || fail
if [ -L "$snapshot_dir/backup.age" ] || [ -L "$snapshot_dir/backup.sha256" ]; then
  fail
fi
...
link_no_follow "$maintenance_manifest" "$manifest_snapshot_dir/maintenance-pod.yaml" >/dev/null 2>&1 || fail
